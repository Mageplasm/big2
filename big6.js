...
function playCard()
{
	...
	switch (lastHandPlayed.code)
	{
		...
		case 'S': 
			userPlayFiveCards(count, cardNums); 
			break;
...
function userPlayFiveCards(count, cardNums) 
{
	var num; 
...
function playUserFiveCards(cardNums, index)
{
	var cards = [];
	var number = []; 
	var suits = []; 
...
{ 
	...
	else 
	{
		...
		/*if ((playerDubs[index].length != 0 && ((randomNum >= 1) && (randomNum <= 3))) || (playerDubs[index].length != 0 && count[index] == 2)) 
		{
			playDouble(0, index);
			removeDubs(index); 
		...
		if (((fiveCardHands[index]['flush'].length != 0 || fiveCardHands[index]['straight'].length != 0) && ((randomNum >= 1) && (randomNum <= 6)))
				|| ((fiveCardHands[index]['flush'].length != 0 || fiveCardHands[index]['straight'].length != 0) && count[index] == 5)) 
		{
